    ...
    tailDateTime.prototype = {
        ...
        renderCalendar: function(){
            ...
            if(_s){
                ...
                // dt.style.cssText = 'top:0;left:0;z-index:999;position:static;visibility:hidden;';
                dt.style.cssText = 'position: static;visibility: visible;opacity: 1;display: block;';
            }
         ...
         */
         handleLabel: function(dt){
            ...
            switch(this.view.type){
                case "days":
                    text = this.__["months"][this.view.date.getMonth()] + ", " + (this.view.date.getFullYear() + 543); break;  // เปลี่ยนเป็น พ.ศ.
                case "months":
                    text = this.view.date.getFullYear() + 543; break;  // เปลี่ยนเป็น พ.ศ.
                case "years":
                    year = parseInt((this.view.date.getFullYear() + 543).toString().slice(0, 3) + "0");  // เปลี่ยนเป็น พ.ศ. และปรับปีให้ถูกต้อง
                    text = year + " - " + (year+10); break;
                case "decades":
                    year = parseInt((this.view.date.getFullYear() + 543).toString().slice(0, 2) + "00");  // เปลี่ยนเป็น พ.ศ. และปรับปีให้ถูกต้อง
         ...
         */
         viewDecades: function(){
            var year = this.view.date.getFullYear(),
                ...
                today = this.con.today? (new Date()).getFullYear() : 0; // แก้ไขการหาปีปัจจุบันให้เป็นปีแบบ คศ
        
            date.setFullYear(year - (year % 10) - 30); // ปรับปีให้เป็นทศวรรษและแปลงเป็น พ.ศ.
        
            for(var c, a, t = [], r = [], i = 1; i <= 16; i++){
                c = 'calendar-decade' + (today >= date.getFullYear() && today <= (date.getFullYear()+9)? ' date-today' : '');
                a = 'data-action="view" data-view="down" data-year="' + date.getFullYear() + '"';
                t.push('<td class="' + c + '" ' + a + '><span class="inner">' + (date.getFullYear() + 543) + " - " + (date.getFullYear() + 552) + '</span></td>'); // แปลงปีเป็น พ.ศ.
         ...
         */
         viewYears: function(){
            var year = this.view.date.getFullYear(),
                date = new Date(this.view.date.getTime()),
                today = this.con.today ? (new Date()).getFullYear() : 0;  // แก้ไขเป็น getFullYear() เนื่องจาก getYear() จะส่งค่ากลับมาเป็นจำนวนปีหลังค.ศ. 1900
        
            // แปลงปีใน date จาก ค.ศ. เป็น พ.ศ.
            date.setFullYear(year - 2);  // ปรับปีให้เป็น พ.ศ. และลดลง 2 ปี
        
            var t = [], r = [];
            for(var i = 1; i <= 16; i++){
                var c = 'calendar-year' + ((date.getFullYear() == today) ? ' date-today' : '');
                var a = 'data-action="view" data-view="down" data-year="' + date.getFullYear() + '"';
                var thaiBuddhistYear = date.getFullYear() + 543;  // แปลงปีในรูปแบบ พ.ศ.
        
                t.push('<td class="' + c + '" ' + a + '><span class="inner">' + thaiBuddhistYear + '</span></td>');
